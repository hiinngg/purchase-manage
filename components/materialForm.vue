<template>
    ...
    >
      <UFormGroup class="mt-2" label="物料编号" name="materialCode">
        <UInput v-model="state.materialCode" autocomplete="off" />
      </UFormGroup>
  ...
  const validate = (state) => {
    ...
    if (!state.materialCode)
      errors.push({ path: "materialCode", message: "不能为空" });
      if (!state.materialName)
      errors.push({ path: "materialName", message: "不能为空" });
      if (!state.materialModel)
      errors.push({ path: "materialModel", message: "不能为空" });
    return errors;
  };
  ...
  async function onSubmit(event) {
   ...
   try {
    const {data,error,status} = await useFetch('/api/material/add',{
      method:'post',
      body:event.data,
      watch:false,
    })
    if(status.value=='success'){
      ...
      if(error.value.data.data.errorCode==1001){
        form.value.setErrors([{message:error.value.data.message,path:'materialCode'}],'materialCode')
      }
    }
   } catch (error) {
